...
public class BlockTransformationUpgradeWrapper extends UpgradeWrapperBase<BlockTransformationUpgradeWrapper, BlockTransformationUpgradeItem> {
			...
			protected void onContentsChanged(int slot, ItemStack previousContents) {
				...
				if (slot == 0) {
					...
					if (inputStack.isEmpty()) {
						upgrade.remove(ModCoreDataComponents.INPUT_ITEM.get());
					} else {
						upgrade.set(ModCoreDataComponents.INPUT_ITEM.get(), SimpleItemContent.copyOf(inputStack));
					}
				}
				save();
			}
		};
		ItemStack inputItem = upgrade.getOrDefault(ModCoreDataComponents.INPUT_ITEM.get(), SimpleItemContent.EMPTY).copy();
	...
	public void setResult(ItemStack result) {
		if (result.isEmpty()) {
			upgrade.remove(ModCoreDataComponents.RESULT_ITEM.get());
			return;
		}

		upgrade.set(ModCoreDataComponents.RESULT_ITEM.get(), SimpleItemContent.copyOf(result));
		save();
	}

	public Optional<SimpleItemContent> getResult() {
		return Optional.ofNullable(upgrade.get(ModCoreDataComponents.RESULT_ITEM.get()));
	}
	...
	public boolean shouldShiftClickIntoStorage() {
		return upgrade.getOrDefault(ModCoreDataComponents.SHIFT_CLICK_INTO_STORAGE.get(), true);
	}

	public void setShiftClickIntoStorage(boolean shiftClickIntoStorage) {
		upgrade.set(ModCoreDataComponents.SHIFT_CLICK_INTO_STORAGE.get(), shiftClickIntoStorage);
		save();
	}

	public boolean shouldRefillInput() {
		return upgrade.getOrDefault(ModCoreDataComponents.REFILL_INPUT.get(), false);
	}

	public void setRefillInput(boolean refillInput) {
		upgrade.set(ModCoreDataComponents.REFILL_INPUT.get(), refillInput);
		save();
	}
