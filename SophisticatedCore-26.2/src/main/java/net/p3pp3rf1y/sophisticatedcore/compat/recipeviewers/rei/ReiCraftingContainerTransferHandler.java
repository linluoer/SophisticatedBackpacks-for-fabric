...
public class ReiCraftingContainerTransferHandler<C extends StorageContainerMenuBase<?>, D extends Display> implements SimpleTransferHandler {
	...
	public Result handle(Context context) {
		...
		List<InputIngredient<ItemStack>> missing = hasItemsIndexed(inventorySlots, inputs);
		if (!missing.isEmpty()) {
		...
		if (recipeTypeId != null) {
			...
			ClientPacketDistributor.sendToServer(new ReiTransferRecipePayload(context.getDisplay().getDisplayLocation().get(), recipeTypeId, save(inputs),
	...
	private static Tag saveInputs(List<InputIngredient<ItemStack>> inputs) {
		...
		for (InputIngredient<ItemStack> input : inputs) {
			...
			innerTag.put("Ingredient",
					EntryIngredient.codec().encodeStart(RegistryHelper.getRegistryAccess().orElseThrow().createSerializationContext(NbtOps.INSTANCE),
							EntryIngredients.ofItemStacks(input.get())).getOrThrow());
	...
	private static List<InputIngredient<ItemStack>> hasItemsIndexed(List<SlotAccessor> inventorySlots, List<InputIngredient<ItemStack>> inputs) {
		Map<ItemStack, Integer> available = new HashMap<>();
		for (SlotAccessor slot : inventorySlots) {
			ItemStack stack = slot.getItemStack();
			if (!stack.isEmpty()) {
				boolean found = false;
				for (Map.Entry<ItemStack, Integer> entry : available.entrySet()) {
		...
		for (InputIngredient<ItemStack> input : inputs) {
			...
			for (ItemStack candidate : input.get()) {
				for (Map.Entry<ItemStack, Integer> entry : available.entrySet()) {
					if (ItemStack.isSameItemSameComponents(entry.getKey(), candidate) && entry.getValue() >= candidate.getCount()) {
						entry.setValue(entry.getValue() - candidate.getCount());
						satisfied = true;
						break;
					}
				}
