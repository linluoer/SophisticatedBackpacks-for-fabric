...
public class StorageWrapperRepository {
	...
	private static final Map<Boolean, Cache<ItemStack, IStorageWrapper>> stackStorageWrappers = new MapMaker().weakKeys().makeMap();
	private static final Map<Boolean, Cache<UUID, IStorageWrapper>> uuidStorageWrappers = new MapMaker().weakKeys().makeMap();
	private static final Map<Boolean, Cache<UUID, Set<IStorageWrapper>>> storageWrappersByUuid = new MapMaker().weakKeys().makeMap();
	...
	private static Cache<ItemStack, IStorageWrapper> getStackStorageWrappers() {
		return stackStorageWrappers.computeIfAbsent(SidedThreadGroups.isServerThread(),
				isServer -> CacheBuilder.newBuilder().expireAfterAccess(10L, TimeUnit.MINUTES).build());
	}

	private static Cache<UUID, IStorageWrapper> getUuidStorageWrappers() {
		return uuidStorageWrappers.computeIfAbsent(SidedThreadGroups.isServerThread(),
				isServer -> CacheBuilder.newBuilder().expireAfterAccess(10L, TimeUnit.MINUTES).build());
	}

	private static Cache<UUID, Set<IStorageWrapper>> getStorageWrappersByUuid() {
		return storageWrappersByUuid.computeIfAbsent(SidedThreadGroups.isServerThread(),
				isServer -> CacheBuilder.newBuilder().expireAfterAccess(10L, TimeUnit.MINUTES).build());
	}
}
