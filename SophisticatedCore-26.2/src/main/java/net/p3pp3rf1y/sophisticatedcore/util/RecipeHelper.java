...
public class RecipeHelper {
	...
	private static List<ItemStack> getUncompactResultItems(Level w, ItemStack itemToUncompact) {
		...
		return safeGetRecipesFor(RecipeType.CRAFTING, craftingInventory.asCraftInput(), w).stream()
				.map(r -> r.value().assemble(craftingInventory.asCraftInput())).toList();
	}
	...
	private static CompactingResult cacheAndGetCompactingResult(CompactedItem compactedItem, CraftingRecipe recipe, CraftingContainer craftingInventory) {
		return getLevel().map(level -> cacheAndGetCompactingResult(compactedItem, recipe, craftingInventory, recipe.assemble(craftingInventory.asCraftInput())))
	...
	private static RecipeMap getRecipeMap(Level level) {
		return level instanceof ServerLevel serverLevel && serverLevel.recipeAccess() instanceof RecipeManager recipeManager
				? recipeManager.recipes : RECIPE_MAP;
	}
	...
	public static ContextMap getContextMap() {
		return getLevel().map(SlotDisplayContext::fromLevel).orElse(new ContextMap.Builder().create(SlotDisplayContext.CONTEXT));
	}
