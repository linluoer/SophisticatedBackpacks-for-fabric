...
public class AlchemyUpgradeWrapper extends UpgradeWrapperBase<AlchemyUpgradeWrapper, AlchemyUpgradeItem> implements ITickableUpgrade {
	...
	public List<AlchemyFilterAttribute> getFilterAttributes() {
		return upgrade.getOrDefault(ModCoreDataComponents.ALCHEMY_FILTER_ATTRIBUTES.get(), getEmptyAttributes());
	}
	...
	private void setUpgradeStackAlchemyAttributes(List<AlchemyFilterAttribute> attributes) {
		upgrade.set(ModCoreDataComponents.ALCHEMY_FILTER_ATTRIBUTES.get(), attributes instanceof ImmutableList ? attributes : ImmutableList.copyOf(attributes));
	}
	...
	static {
		...
		addItemDefinition(new AlchemyItemDefinition(stack -> stack.getItem() == Items.SPLASH_POTION, AlchemyUpgradeWrapper::getDefaultConditionForPotion,
				AlchemyUpgradeWrapper::shouldApplyPotionEffectsTo, AlchemyUpgradeWrapper::stackPotionEffectsMatch, (stack, livingEntity) -> {
					...
					ThrownSplashPotion thrownPotion = new ThrownSplashPotion(level, livingEntity.getX() + livingEntity.getBbWidth() / 2,
							livingEntity.getY() + livingEntity.getEyeHeight(), livingEntity.getZ() + livingEntity.getBbWidth() / 2, stack);
		...
		addItemDefinition(new AlchemyItemDefinition(stack -> stack.getItem() == Items.GOLDEN_APPLE, stack -> AlchemyCondition.ALWAYS,
				...
				ItemStack::getUseDuration, (stack, livingEntity) -> {
		...
		addItemDefinition(new AlchemyItemDefinition(AlchemyUpgradeWrapper::isEffectAffectingConsumable, AlchemyUpgradeWrapper::getDefaultConditionForConsumable,
				(le, potionStack, matchAllEffects, matchEffectAmplifier) -> {
	...
	private static boolean isEffectAffectingConsumable(ItemStack stack) {
		...
		List<ConsumeEffect> consumeEffects = consumable.onConsumeEffects();
		for (ConsumeEffect consumeEffect : consumeEffects) {
			if (consumeEffect instanceof ClearAllStatusEffectsConsumeEffect || consumeEffect instanceof RemoveStatusEffectsConsumeEffect
					|| consumeEffect instanceof ApplyStatusEffectsConsumeEffect) {
				return true;
			}
		}
	...
	private static boolean matchEffectIn(PotionContents potionContents, MobEffectInstance filterEffectInstance, boolean matchEffectDuration,
			boolean matchEffectAmplifier) {
		for (MobEffectInstance effectInstance : potionContents.getAllEffects()) {
			Holder<MobEffect> effect = effectInstance.getEffect();
			if (effect == filterEffectInstance.getEffect() && (!matchEffectDuration || effectInstance.getDuration() == filterEffectInstance.getDuration())
	...
	public void setMatchAllEffects(boolean matchAllEffects) {
		upgrade.set(ModCoreDataComponents.MATCH_ALL_EFFECTS.get(), matchAllEffects);
		save();
	}

	public boolean shouldMatchAllEffects() {
		return upgrade.getOrDefault(ModCoreDataComponents.MATCH_ALL_EFFECTS.get(), true);
	}

	public boolean shouldMatchEffectDuration() {
		return upgrade.getOrDefault(ModCoreDataComponents.MATCH_EFFECT_DURATION.get(), true);
	}

	public void setMatchEffectDuration(boolean matchEffectDuration) {
		upgrade.set(ModCoreDataComponents.MATCH_EFFECT_DURATION.get(), matchEffectDuration);
		save();
	}

	public boolean shouldMatchEffectAmplifier() {
		return upgrade.getOrDefault(ModCoreDataComponents.MATCH_EFFECT_AMPLIFIER.get(), true);
	}

	public void setMatchEffectAmplifier(boolean matchEffectAmplifier) {
		upgrade.set(ModCoreDataComponents.MATCH_EFFECT_AMPLIFIER.get(), matchEffectAmplifier);
		save();
	}

	public EntityMatch getEntityMatch() {
		return upgrade.getOrDefault(ModCoreDataComponents.ENTITY_MATCH.get(), EntityMatch.PLAYERS_AND_ENTITIES);
	}

	public void setEntityMatch(EntityMatch entityMatch) {
		upgrade.set(ModCoreDataComponents.ENTITY_MATCH.get(), entityMatch);
		save();
	}
