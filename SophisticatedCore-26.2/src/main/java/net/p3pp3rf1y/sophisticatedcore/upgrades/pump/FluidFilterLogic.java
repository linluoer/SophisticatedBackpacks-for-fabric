...
public class FluidFilterLogic {
	...
	private void deserializeFluidFilters() {
		List<SimpleFluidContent> deserializedFilters = upgrade.getOrDefault(ModCoreDataComponents.FLUID_FILTERS.get(), Collections.emptyList());
	...
	private void serializeFluidFilters() {
		upgrade.set(ModCoreDataComponents.FLUID_FILTERS.get(), fluidFilters.stream().map(SimpleFluidContent::copyOf).toList());
	}
}
