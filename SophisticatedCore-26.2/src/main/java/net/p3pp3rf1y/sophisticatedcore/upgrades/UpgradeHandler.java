...
public class UpgradeHandler extends ItemStacksResourceHandler implements ISlotStackAccessor, IndexModifier<ItemResource> {
	...
	public UpgradeHandler(int numberOfUpgradeSlots, IStorageWrapper storageWrapper, ContainerContents containerContents, Runnable contentsSaveHandler,
		...
		if (SidedThreadGroups.isServerThread() && storageWrapper.getRenderDataHandler().getUpgradeItems().size() != size()) {
	...
	protected void onContentsChanged(int slot, ItemStack previousContents) {
		...
		if (SidedThreadGroups.isServerThread() && !currentStack.isEmpty()) {
			onUpgradeAdded(slot);
		}
	}
	...
	public void setStackInSlot(int slot, ItemStack stack) {
		...
		if (SidedThreadGroups.isServerThread() && !originalStack.isEmpty() && !runningOnBeforeRemovedOnSlots.contains(slot)) {
