...
public abstract class BlockConverterRecipeControl<R extends SingleItemRecipe, RC extends BlockConverterRecipeContainer<R, ?, RC, ?>> extends WidgetBase {
	...
	public BlockConverterRecipeControl(StorageScreenBase<?> screen, BlockConverterRecipeContainer<R, ?, RC, ?> container, Position position,
			boolean renderResultCount) {
	...
	public void moveSlotsToView() {
		...
		inputSlot.x = x + getResultAreaCenteredX(16) - screen.leftPos;
		inputSlot.y = y - screen.topPos + 1;
		Slot outputSlot = container.getOutputSlot();
		outputSlot.x = x + getResultAreaCenteredX(16) - screen.leftPos;
	...
	private void drawRecipesItems(GuiGraphicsExtractor guiGraphics, int listInnerLeftX, int top, int recipeIndexOffsetMax) {
		...
		for (int displayIndex = recipeIndexOffset; displayIndex < recipeIndexOffsetMax && displayIndex < sortedRecipeIndexes.size(); ++displayIndex) {
			...
			int l = j / 4;
			int i1 = top + l * 18 + 2;
			ItemStack resultItem = list.get(sortedRecipeIndexes.get(displayIndex)).value().result.create();
			GuiHelper.renderItemInGUI(guiGraphics, minecraft, resultItem, k, i1, renderResultCount && resultItem.getCount() > 1,
					String.valueOf(resultItem.getCount()));
		}
	}
	...
	private void renderHoveredTooltip(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		if (shouldShowBrowseButton() && browseButton.isMouseOver(mouseX, mouseY)) {
			browseButton.extractTooltip(screen, guiGraphics, mouseX, mouseY);
			return;
		}
		if (hasItemsInInputSlot) {
			...
			for (int displayIndex = recipeIndexOffset; displayIndex < k && displayIndex < sortedRecipeIndexes.size(); ++displayIndex) {
				...
				if (mouseX >= recipeLeftX && mouseX < recipeLeftX + 16 && mouseY >= k1 && mouseY < k1 + 18) {
					extractTooltip(guiGraphics, list.get(sortedRecipeIndexes.get(displayIndex)).value().result.create(), mouseX, mouseY);
				}
			}
		}
	}
	...
	private void extractTooltip(GuiGraphicsExtractor guiGraphics, ItemStack itemStack, int mouseX, int mouseY) {
		guiGraphics.setComponentTooltipForNextFrame(this.font, Screen.getTooltipFromItem(minecraft, itemStack), mouseX, mouseY);
	}
	...
	private void openLargeResultSelector() {
		...
		for (int i : getSortedRecipeIndexes()) {
			results.add(new LargeResultSelector.ResultEntry(i, recipes.get(i).value().result.create(), container.isRecentResult(i)));
		}
		screen.setModalOverlay(
				new LargeResultSelector(screen, results, container.getSelectedRecipe(), this::selectRecipeFromLargeResultSelector, getSelectRecipeSound()));
	}
