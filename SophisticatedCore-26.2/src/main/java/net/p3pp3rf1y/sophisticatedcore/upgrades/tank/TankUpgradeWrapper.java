...
public class TankUpgradeWrapper extends UpgradeWrapperBase<TankUpgradeWrapper, TankUpgradeItem>
	...
	private static final Map<ItemStack, Function<ItemAccess, ResourceHandler<FluidResource>>> CUSTOM_FLUIDHANDLER_FACTORIES = Map.of(
			...
			itemAccess -> new SwapEmptyFluidContainerHandler.Full(
					itemAccess, Items.GLASS_BOTTLE, new ItemStack(Items.EXPERIENCE_BOTTLE), XpHelper.experienceToLiquid(8), ModFluids.XP_STILL.get()),
			PotionContents.createItemStack(Items.POTION, Potions.WATER),
			itemAccess -> new SwapEmptyFluidContainerHandler.Full(
					itemAccess, Items.GLASS_BOTTLE, PotionContents.createItemStack(Items.POTION, Potions.WATER), 250, Fluids.WATER),
			new ItemStack(Items.GLASS_BOTTLE),
			itemAccess -> new SwapEmptyFluidContainerHandler.Empty(itemAccess, Items.GLASS_BOTTLE,
					new SwapEmptyFluidContainerHandler.FullContainerDefinition(new ItemStack(Items.EXPERIENCE_BOTTLE), XpHelper.experienceToLiquid(8),
							ModFluids.XP_STILL.get()),
					new SwapEmptyFluidContainerHandler.FullContainerDefinition(PotionContents.createItemStack(Items.POTION, Potions.WATER), 250,
							Fluids.WATER)));
	...
	public static SimpleFluidContent getContents(ItemStack upgrade) {
		return upgrade.getOrDefault(ModCoreDataComponents.FLUID_CONTENTS.get(), SimpleFluidContent.EMPTY);
	}
	...
	private boolean isValidFluidHandler(ResourceHandler<FluidResource> fluidHandler, boolean isOutput) {
		...
		for (int tank = 0; tank < fluidHandler.size(); tank++) {
			...
			int capacity = fluidHandler.getCapacityAsInt(tank, fluidInTank);
			if (isOutput && amount < capacity
	...
	private boolean hasNoMatchingFluid(ResourceHandler<FluidResource> fluidHandler) {
		if (contents.isEmpty()) {
			for (int tank = 0; tank < fluidHandler.size(); tank++) {
				if (!fluidHandler.getResource(tank).isEmpty()) {
					return false;
				}
			}
			return true;
		...
		for (int tank = 0; tank < fluidHandler.size(); tank++) {
			if (fluidHandler.getResource(tank).matches(contents)) {
				return false;
			}
		}
		return true;
	...
	public void forceUpdateTankRenderData() {
		updateTankRenderDataCallback.accept(new RenderData.TankRenderData(contents.copy(),
	...
	private int getMaxInOut() {
		return (int) Math.max(FluidType.BUCKET_VOLUME, upgradeItem.getTankUpgradeConfig().maxInputOutput.get() * storageWrapper.getNumberOfSlotRows()
				* upgradeItem.getAdjustedStackMultiplier(storageWrapper));
	}
	...
	private void serializeContents() {
		upgrade.set(ModCoreDataComponents.FLUID_CONTENTS.get(), SimpleFluidContent.copyOf(contents));
		save();
	...
	public void tick(@Nullable Entity entity, Level level, BlockPos pos) {
		...
		// INPUT_SLOT: only drain (full container -> empty container + fluid into tank)
		if (!inputStack.isEmpty()) {
			didSomething = drainStack(inputStack, INPUT_SLOT, INPUT_RESULT_SLOT);
		}

		// OUTPUT_SLOT: only fill (empty container + fluid from tank -> full container)
		if (!didSomething && !outputStack.isEmpty()) {
			didSomething = fillStack(outputStack, OUTPUT_SLOT, OUTPUT_RESULT_SLOT);
		}
	...
	private boolean drainStack(ItemStack stackToDrain, int inputSlot, int resultSlot) {
		...
		try (Transaction tx = Transaction.openRoot()) {
			...
			Optional<ResourceHandler<FluidResource>> fluidHandlerOpt = getFluidHandler(stackToDrain, itemAccess);
			return fluidHandlerOpt.map(fluidHandler -> {
				int drained = drainHandler(fluidHandler, tx);
				if (drained > 0) {
					boolean noMatching = hasNoMatchingFluid(fluidHandler);
					if (noMatching) {
						int inserted = inventory.insert(resultSlot, itemAccess.getResource(), itemAccess.getAmount(), tx);
						if (inserted == itemAccess.getAmount()) {
							tx.commit();
							inventory.setStackInSlot(inputSlot,
									stackToDrain.getCount() == 1 ? ItemStack.EMPTY : stackToDrain.copyWithCount(stackToDrain.getCount() - 1));
							return true;
					...
					tx.commit();
					inventory.setStackInSlot(inputSlot, itemAccess.getStack());
					return true;
				}
				return false;
			}).orElse(false);
	...
	private Optional<ResourceHandler<FluidResource>> getFluidHandler(ItemStack stack, ItemAccess itemAccess) {
		ResourceHandler<FluidResource> result = itemAccess.getCapability(Capabilities.Item.FLUID);
		if (result != null) {
	...
	private boolean fillStack(ItemStack stackToFill, int outputSlot, int resultSlot) {
		...
		try (Transaction tx = Transaction.openRoot()) {
			...
			return getFluidHandler(stackToFill, itemAccess).map(fluidHandler -> {
				int filled = fillHandler(fluidHandler, tx);
				if (filled > 0) {
					if (matchingTankIsFull(fluidHandler)) {
						if (inventory.insert(resultSlot, itemAccess.getResource(), itemAccess.getAmount(), tx) == itemAccess.getAmount()) {
							tx.commit();
							inventory.setStackInSlot(outputSlot,
									stackToFill.getCount() == 1 ? ItemStack.EMPTY : stackToFill.copyWithCount(stackToFill.getCount() - 1));
							return true;
					...
					tx.commit();
					inventory.setStackInSlot(outputSlot, itemAccess.getStack());
					return true;
				}
				return false;
			}).orElse(false);
	...
	private static Optional<ResourceHandler<FluidResource>> getCustomFluidHandler(ItemStack stack, ItemAccess itemAccess) {
		return CUSTOM_FLUIDHANDLER_FACTORIES.entrySet().stream().filter(e -> ItemStack.isSameItemSameComponents(stack, e.getKey()))
				.map(e -> e.getValue().apply(itemAccess)).findFirst();
	}
	...
	public int drainHandler(ResourceHandler<FluidResource> fluidHandler, TransactionContext tx) {
		boolean valid = isValidFluidHandler(fluidHandler, false);
		if (valid) {
			...
			int inserted = insert(resource, extracted, tx, true);
			return inserted;
		}
		return 0;
	}
	...
	public class TankComponentItemHandler extends ItemAccessItemHandler {
		...
		public boolean isValid(int index, ItemResource resource) {
			if (index == INPUT_SLOT) {
				// INPUT_SLOT: only accept full containers (drain operation)
				if (resource.isEmpty()) {
					return true;
				}
				boolean drainValid = hasValidFluidHandler(resource, false);
				return drainValid;
			}
			if (index == OUTPUT_SLOT) {
				// OUTPUT_SLOT: only accept empty containers (fill operation)
				if (resource.isEmpty()) {
					return true;
				}
				boolean fillValid = hasValidFluidHandler(resource, true);
				return fillValid;
			}
			boolean result = index == INPUT_RESULT_SLOT || index == OUTPUT_RESULT_SLOT;
			return result;
		}

		private boolean hasValidFluidHandler(ItemResource resource, boolean isOutput) {
			Optional<ResourceHandler<FluidResource>> handler = getFluidHandler(resource.toStack(), ItemAccess.forStack(resource.toStack()));
			return handler.map(fluidHandler -> {
				boolean valid = isValidFluidHandler(fluidHandler, isOutput);
				return valid;
			}).orElse(false);
		}
	...
	private static abstract class SwapEmptyFluidContainerHandler implements ResourceHandler<FluidResource> {
		...
		public static class Full extends SwapEmptyFluidContainerHandler {
			public Full(ItemAccess itemAccess, Item empty, ItemStack full, int capacity, Fluid validFluid) {
				super(itemAccess, empty, new FluidStack(validFluid, capacity),
						new FullContainerDefinition(full, capacity, new FluidStack(validFluid, capacity)));
			}
		}
		...
		private Optional<FullContainerDefinition> getMatchingDefinition() {
			return fullContainers.entrySet().stream().filter(e -> FluidStack.isSameFluidSameComponents(e.getKey(), contents)).map(Map.Entry::getValue)
					.findFirst();
		}
		...
		private Optional<FullContainerDefinition> findFirstFullContainer(FluidResource resource) {
			return fullContainers.entrySet().stream().filter(e -> resource.matches(e.getKey())).findFirst().map(Map.Entry::getValue);
		}

		@Override
		public int insert(FluidResource resource, int amount, TransactionContext tx) {
			return insert(0, resource, amount, tx);
		}
