...
public class AutoCookingUpgradeWrapper<W extends AutoCookingUpgradeWrapper<W, U, R>, U extends UpgradeItemBase<W> & IAutoCookingUpgradeItem, R extends AbstractCookingRecipe>
	...
	public AutoCookingUpgradeWrapper(IStorageWrapper storageWrapper, ItemStack upgrade, Consumer<ItemStack> upgradeSaveHandler, RecipeType<R> recipeType,
		...
		fuelFilterLogic = new FilterLogic(upgrade, upgradeSaveHandler, autoCookingUpgradeConfig.fuelFilterSlots.get(),
				s -> WorldHelper.getFuelValues().burnDuration(s) > 0, ModCoreDataComponents.FUEL_FILTER_ATTRIBUTES);
		...
		isValidFuel = s -> WorldHelper.getFuelValues().burnDuration(s) > 0 && fuelFilterLogic.matchesFilter(s);
	...
	private void tryPushingOutput() {
		...
		if (!output.isEmpty()) {
			int moved = moveFromCookingSlot(inventory, CookingLogic.COOK_OUTPUT_SLOT, output);
			if (moved > 0) {
				tryPushingXpToTanks();
			} else {
		...
		if (!fuel.isEmpty() && WorldHelper.getFuelValues().burnDuration(fuel) <= 0) {
			moveFromCookingSlot(inventory, CookingLogic.FUEL_SLOT, fuel);
		}
	}

	private int moveFromCookingSlot(ITrackedContentsItemResourceHandler inventory, int slot, ItemStack stack) {
		ItemResource resource = ItemResource.of(stack);
		try (Transaction tx = Transaction.openRoot()) {
			int extracted = cookingLogic.getCookingInventory().extract(slot, resource, stack.getCount(), tx);
			if (extracted <= 0) {
				return 0;
			}
			int inserted = inventory.insert(resource, extracted, tx);
	...
	private void tryPullingInput() {
		...
		if (tryPullingGetUnsucessful(CookingLogic.COOK_INPUT_SLOT, isValidInput)) {
	...
	private void tryPullingFuel() {
		...
		if (tryPullingGetUnsucessful(CookingLogic.FUEL_SLOT, isValidFuel)) {
			fuelCooldown = NO_INVENTORY_SPACE_COOLDOWN;
		}
	}

	private boolean tryPullingGetUnsucessful(int slot, Predicate<ItemStack> isItemValid) {
		ItemStack stack = cookingLogic.getCookingInventory().getStackInSlot(slot);
		ItemStack toExtract;
		ITrackedContentsItemResourceHandler inventory = storageWrapper.getInventoryForUpgradeProcessing();
		if (stack.isEmpty()) {
			AtomicReference<ItemStack> ret = new AtomicReference<>(ItemStack.EMPTY);
			InventoryHelper.iterate(inventory, (index, candidate) -> {
				if (!candidate.isEmpty() && isItemValid.test(candidate)) {
					ret.set(candidate.copy());
				}
			}, () -> !ret.get().isEmpty());
			if (ret.get().isEmpty()) {
				return true;
		...
		ItemResource resource = ItemResource.of(toExtract);
		int amount = Math.min(toExtract.getMaxStackSize() - stack.getCount(), cookingLogic.getCookingInventory().getCapacityAsInt(slot, resource) - stack.getCount());
		if (slot == CookingLogic.FUEL_SLOT && toExtract.getItem().getCraftingRemainder() != null) {
			amount = Math.min(amount, 1 - stack.getCount());
		}
		if (amount <= 0) {
			return true;
		}
		try (Transaction tx = Transaction.openRoot()) {
			int extracted = inventory.extract(resource, amount, tx);
			if (extracted <= 0) {
				return true;
			}
			int inserted = cookingLogic.getCookingInventory().insert(slot, resource, extracted, tx);
			if (inserted == extracted) {
				tx.commit();
				return false;
			}
