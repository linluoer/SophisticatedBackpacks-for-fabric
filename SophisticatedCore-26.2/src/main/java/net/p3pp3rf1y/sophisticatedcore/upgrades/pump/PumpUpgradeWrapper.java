...
public class PumpUpgradeWrapper extends UpgradeWrapperBase<PumpUpgradeWrapper, PumpUpgradeItem> implements ITickableUpgrade {
	...
	private Optional<Integer> handleInWorldInteractions(ResourceHandler<FluidResource> storageFluidHandler, @Nullable Entity entity, Level level,
			BlockPos pos) {
	...
	private boolean placeFluidInWorld(Level level, ResourceHandler<FluidResource> storageFluidHandler, Direction dir, BlockPos offsetPos, Player player) {
		if (dir != Direction.UP) {
	...
	private Optional<Integer> fillFromBlockInRange(Level level, BlockPos basePos, ResourceHandler<FluidResource> storageFluidHandler, Player player) {
	...
	private boolean fillFromBlock(Level level, BlockPos pos, ResourceHandler<FluidResource> storageFluidHandler, Player player) {
		...
		if (!fluidState.isEmpty() && fluidState.isSource()) {
			...
			if (block instanceof BucketPickup bucketPickup) {
				return pickupBlock(level, pos, storageFluidHandler, bucketPickup, fluidState, state, player);
			}
			ResourceHandler<FluidResource> fluidHandler = CapabilityHelper.getFromCapability(level, pos, Capabilities.Block.FLUID, null, Function.identity(), null);
	...
	private static boolean pickupBlock(Level level, BlockPos pos, ResourceHandler<FluidResource> storageFluidHandler, BucketPickup bucketPickup,
			FluidState fluidState, BlockState state, Player player) {
	...
	public void setIsInput(boolean input) {
		upgrade.set(ModCoreDataComponents.IS_INPUT.get(), input);
		save();
	}

	public boolean isInput() {
		return upgrade.getOrDefault(ModCoreDataComponents.IS_INPUT.get(), true);
	}
	...
	public void setInteractWithHand(boolean interactWithHand) {
		upgrade.set(ModCoreDataComponents.INTERACT_WITH_HAND.get(), interactWithHand);
		save();
	}

	public boolean shouldInteractWithHand() {
		return upgrade.getOrDefault(ModCoreDataComponents.INTERACT_WITH_HAND.get(), upgradeItem.getInteractWithHandDefault());
	}

	public void setInteractWithWorld(boolean interactWithWorld) {
		upgrade.set(ModCoreDataComponents.INTERACT_WITH_WORLD.get(), interactWithWorld);
		save();
	}

	public boolean shouldInteractWithWorld() {
		return upgrade.getOrDefault(ModCoreDataComponents.INTERACT_WITH_WORLD.get(), upgradeItem.getInteractWithWorldDefault());
	}

	public void setInteractWithFluidHandlers(boolean interactWithFluidHandlers) {
		upgrade.set(ModCoreDataComponents.INTERACT_WITH_FLUID_HANDLERS.get(), interactWithFluidHandlers);
		save();
	}

	public boolean shouldInteractWithFluidHandlers() {
		return upgrade.getOrDefault(ModCoreDataComponents.INTERACT_WITH_FLUID_HANDLERS.get(), upgradeItem.getInteractWithFluidHandlersDefault());
	}
}
