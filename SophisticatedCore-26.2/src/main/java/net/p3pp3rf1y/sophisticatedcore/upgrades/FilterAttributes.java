...
public record FilterAttributes(Set<TagKey<Item>> tagKeys, boolean isAllowList, boolean matchDurability, boolean matchComponents, PrimaryMatch primaryMatch,
	...
	public FilterAttributes(Set<TagKey<Item>> tagKeys, boolean isAllowList, boolean matchDurability, boolean matchComponents, PrimaryMatch primaryMatch,
			boolean matchAnyTag, ItemContainerContents filterItems, boolean filterByStorage, boolean filterByInventory) {
		this.tagKeys = Set.copyOf(tagKeys);
	...
	protected static class CopyBuilder {
		...
		public CopyBuilder setFilterItem(int slot, ItemStack filterItem) {
			NonNullList<ItemStack> currentItems = CodecHelper.toMutableNonnullItemStackList(filterItems.allItemsCopyStream().toList());
			int targetSize = Math.max(slot + 1, currentItems.size());
			NonNullList<ItemStack> items = NonNullList.withSize(targetSize, ItemStack.EMPTY);
			for (int i = 0; i < currentItems.size() && i < targetSize; i++) {
				items.set(i, currentItems.get(i));
			}
