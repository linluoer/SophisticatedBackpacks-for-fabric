...
public class VoidUpgradeWrapper extends UpgradeWrapperBase<VoidUpgradeWrapper, VoidUpgradeItem>
	...
	private void setFromLegacyComponent() {
		if (upgrade.has(ModCoreDataComponents.LEGACY_SHOULD_VOID_OVERFLOW.get())) {
			VoidType migratedVoidType = upgrade.get(ModCoreDataComponents.LEGACY_SHOULD_VOID_OVERFLOW.get()) ? VoidType.SLOT_OVERFLOW : VoidType.ALWAYS;
			upgrade.remove(ModCoreDataComponents.LEGACY_SHOULD_VOID_OVERFLOW.get());
	...
	public void setVoidType(VoidType voidType) {
		...
		upgrade.set(ModCoreDataComponents.VOID_TYPE.get(), voidType);
		save();
	}

	public void setVoidOverflowDefaultOrLoadFromNbt(VoidType voidOverflowDefault) {
		VoidType vt = upgrade.getOrDefault(ModCoreDataComponents.VOID_TYPE.get(), voidOverflowDefault);
