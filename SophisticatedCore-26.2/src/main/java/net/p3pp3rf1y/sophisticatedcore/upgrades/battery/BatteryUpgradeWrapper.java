...
public class BatteryUpgradeWrapper extends UpgradeWrapperBase<BatteryUpgradeWrapper, BatteryUpgradeItem>
	...
	public static int getEnergyStored(ItemStack upgrade) {
		return upgrade.getOrDefault(ModCoreDataComponents.ENERGY_STORED.get(), 0);
	}
	...
	private void serializeEnergyStored() {
		upgrade.set(ModCoreDataComponents.ENERGY_STORED.get(), energyStored);
		save();
	...
	public void tick(@Nullable Entity entity, Level level, BlockPos pos) {
		if (energyStored < getCapacityAsLong()) {
			...
			if (!energyContainer.isEmpty()) {
				...
				EnergyHandler energyHandler = itemAccess.getCapability(Capabilities.Item.ENERGY);
		...
		if (energyStored > 0) {
			...
			if (!energyContainer.isEmpty()) {
				ItemAccess itemAccess = ItemAccess.forStack(energyContainer);
				EnergyHandler energyStorage = itemAccess.getCapability(Capabilities.Item.ENERGY);
	...
	private class BatteryComponentItemHandler extends ItemAccessItemHandler {
		...
		private boolean isEnergyHandler(ItemAccess itemAccess) {
			return itemAccess.getCapability(Capabilities.Item.ENERGY) != null;
		}
		...
		public void setStackInSlot(int slot, ItemStack stack) {
			ItemContainerContents contents = getContents(itemAccess.getResource());
			int contentsSize = (int) contents.allItemsCopyStream().count();
			NonNullList<ItemStack> list = NonNullList.withSize(Math.max(contentsSize, this.size), ItemStack.EMPTY);
