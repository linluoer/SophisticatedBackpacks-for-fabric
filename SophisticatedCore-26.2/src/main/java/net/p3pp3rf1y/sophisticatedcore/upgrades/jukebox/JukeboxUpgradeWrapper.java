...
public class JukeboxUpgradeWrapper extends UpgradeWrapperBase<JukeboxUpgradeWrapper, JukeboxUpgradeItem> implements ITickableUpgrade {
	...
	protected JukeboxUpgradeWrapper(IStorageWrapper storageWrapper, ItemStack upgrade, Consumer<ItemStack> upgradeSaveHandler) {
		...
		isPlaying = upgrade.getOrDefault(ModCoreDataComponents.IS_PLAYING.get(), false);
	}

	public boolean isShuffleEnabled() {
		return upgrade.getOrDefault(ModCoreDataComponents.SHUFFLE.get(), false);
	}

	public void setShuffleEnabled(boolean shuffleEnabled) {
		upgrade.set(ModCoreDataComponents.SHUFFLE.get(), shuffleEnabled);
		save();
	...
	public RepeatMode getRepeatMode() {
		return upgrade.getOrDefault(ModCoreDataComponents.REPEAT_MODE.get(), RepeatMode.NO);
	}

	public void setRepeatMode(RepeatMode repeatMode) {
		upgrade.set(ModCoreDataComponents.REPEAT_MODE.get(), repeatMode);
		save();
	}
	...
	public int getDiscSlotActive() {
		return upgrade.getOrDefault(ModCoreDataComponents.DISC_SLOT_ACTIVE.get(), -1);
	}

	private void setDiscSlotActive(int discSlotActive) {
		upgrade.set(ModCoreDataComponents.DISC_SLOT_ACTIVE.get(), discSlotActive);
		save();
	}
	...
	private void playDisc() {
		...
		storageWrapper.getContentsUuid().ifPresent(storageUuid -> DiscHandlerRegistry.findHandler(disc).ifPresent(handler -> {
			...
			handler.getMusicLengthInTicks(disc, level)
					.ifPresent(lengthInTicks -> upgrade.set(ModCoreDataComponents.DISC_FINISH_TIME.get(), level.getGameTime() + lengthInTicks));
		}));
		setIsPlaying(true);
	}
	...
	private void setIsPlaying(boolean playing) {
		isPlaying = playing;
		upgrade.set(ModCoreDataComponents.IS_PLAYING.get(), playing);
		if (isPlaying) {
	...
	public void stop(LivingEntity entity) {
		...
		setIsPlaying(false);
		upgrade.remove(ModCoreDataComponents.DISC_FINISH_TIME.get());
		setDiscSlotActive(-1);
		playlist.clear();
		history.clear();
	}
	...
	public long getDiscFinishTime() {
		return upgrade.getOrDefault(ModCoreDataComponents.DISC_FINISH_TIME.get(), 0L);
	}
}
