...
public class TankInventoryControl extends UpgradeInventoryControlBase {
	...
	public boolean handleMouseReleased(MouseButtonEvent event) {
		int tankLeftScreen = screen.leftPos + getTankLeft();
		int tankRightScreen = tankLeftScreen + 18;
		int tankTopScreen = screen.topPos + pos.y();
		int tankBottomScreen = tankTopScreen + height;
		ItemStack cursorStack = screen.getMenu().getCarried();

		boolean inX = event.x() >= tankLeftScreen && event.x() < tankRightScreen;
		boolean inY = event.y() >= tankTopScreen && event.y() < tankBottomScreen;
	...
	private void extractFluid(GuiGraphicsExtractor guiGraphics) {
		...
		int color = fluidModel.tintSource() instanceof FluidTintSource fluidTintSource
				? fluidTintSource.getTintColor(FluidResource.of(contents))
				: fluidModel.tintSource() != null ? fluidModel.tintSource().color(fluidState.createLegacyBlock()) : -1;
		int fluidY = pos.y() + 1 + height - 2 - displayLevel;
