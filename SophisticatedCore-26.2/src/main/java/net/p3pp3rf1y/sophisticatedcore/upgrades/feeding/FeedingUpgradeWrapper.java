...
public class FeedingUpgradeWrapper extends UpgradeWrapperBase<FeedingUpgradeWrapper, FeedingUpgradeItem> implements ITickableUpgrade, IFilteredUpgrade {
	...
	public void tick(@Nullable Entity entity, Level level, BlockPos pos) {
		...
		if (!(entity instanceof Player)) {
			...
			level.getEntities(EntityTypes.PLAYER, new AABB(pos).inflate(FEEDING_RANGE), p -> true)
			...
			if (singleItemCopy.use(level, player, InteractionHand.MAIN_HAND) == InteractionResult.CONSUME) {
				...
				if (!resultItem.isEmpty()) {
					...
					if (inserted < resultItem.getCount()) {
						...
						CapabilityHelper.runOnCapability(player, Capabilities.Entity.ITEM, null,
	...
	public HungerLevel getFeedAtHungerLevel() {
		return upgrade.getOrDefault(ModCoreDataComponents.FEED_AT_HUNGER_LEVEL.get(), HungerLevel.HALF);
	}

	public void setFeedAtHungerLevel(HungerLevel hungerLevel) {
		upgrade.set(ModCoreDataComponents.FEED_AT_HUNGER_LEVEL.get(), hungerLevel);
		save();
	}

	public boolean shouldFeedImmediatelyWhenHurt() {
		return upgrade.getOrDefault(ModCoreDataComponents.FEED_IMMEDIATELY_WHEN_HURT.get(), true);
	}

	public void setFeedImmediatelyWhenHurt(boolean feedImmediatelyWhenHurt) {
		upgrade.set(ModCoreDataComponents.FEED_IMMEDIATELY_WHEN_HURT.get(), feedImmediatelyWhenHurt);
		save();
	}
}
