...
public class InventoryScrollPanel extends ScrollPanel {
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClicked) {
		if (event.button() == 0 && isMouseOver(event.x(), event.y())) {
			// If the click is on a storage slot, let the normal slot click logic handle it
			// instead of consuming it as a scrollbar drag.
			if (getHoveredSlot(event.x(), event.y()).isPresent()) {
				return false;
			}
