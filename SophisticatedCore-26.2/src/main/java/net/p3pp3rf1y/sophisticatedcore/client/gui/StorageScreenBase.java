...
public abstract class StorageScreenBase<S extends StorageContainerMenuBase<?>> extends AbstractContainerScreen<S>
		implements
			InventoryScrollPanel.IInventoryScreen {
	...
	private static final int DISABLED_FILTER_SLOT_COLOR = 0x80_000000;
	...
	protected void updatePlayerSlotsPositions() {
		...
		for (int i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				...
				Slot slot = getMenu().getSlot(
						getMenu().getInventorySlotsSize() - getMenu().getExtraSlots().size() - StorageContainerMenuBase.NUMBER_OF_PLAYER_SLOTS + slotIndex);
				slot.x = xPosition;
				slot.y = yPosition;
			}
			yPosition += 18;
		...
		for (int slotIndex = 0; slotIndex < 9; ++slotIndex) {
			...
			Slot slot = getMenu().getSlot(
					getMenu().getInventorySlotsSize() - getMenu().getExtraSlots().size() - StorageContainerMenuBase.NUMBER_OF_PLAYER_SLOTS + 3 * 9 + slotIndex);
			slot.x = xPosition;
			slot.y = yPosition;
		}
	}
	...
	protected void addSearchBox() {
		...
		noResultsLabel = new Label(new Position(leftPos + 7, topPos + 18),
	...
	private void addTransferButtons() {
		transferToStorageButton = new TransferButton(filterByContents -> getMenu().transferItemsToStorage(filterByContents),
		...
		transferToInventoryButton = new TransferButton(filterByContents -> getMenu().transferItemsToPlayerInventory(filterByContents),
	...
	private void updateInventoryScrollPanel() {
		...
		if (numberOfVisibleRows < getMenu().getNumberOfRows()) {
			inventoryScrollPanel = new InventoryScrollPanel(Minecraft.getInstance(), this, 0, getMenu().getNumberOfStorageInventorySlots(), getSlotsOnLine(),
					numberOfVisibleRows * 18, topPos + 17, leftPos + 7);
		...
		for (int slot = 0; slot < numberOfUpgradeSlots; slot++) {
			if (menu.canDisableUpgrade(slot)) {
				...
				ToggleButton<Boolean> upgradeSwitch = new ToggleButton<>(new Position(leftPos - 22, switchTop), ButtonDefinitions.UPGRADE_SWITCH,
						button -> getMenu().setUpgradeEnabled(finalSlot, !getMenu().getUpgradeEnabled(finalSlot)),
						() -> getMenu().getUpgradeEnabled(finalSlot)) {
					@Override
					...
					public void extractTooltip(Screen screen, GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
						if (menu.isUpgradeRunnable(finalSlot)) {
							...
							if (isMouseOver(mouseX, mouseY)) {
								GuiHelper.extractTooltip(screen, guiGraphics, ButtonDefinitions.UPGRADE_SWITCH_INACTIVE.getTooltip(), mouseX, mouseY);
							}
						}
					}
	...
	private Position getSortButtonsPosition(SortButtonsPosition sortButtonsPosition) {
		return switch (sortButtonsPosition) {
			case BELOW_UPGRADES -> new Position(leftPos - UPGRADE_INVENTORY_OFFSET - 2, topPos + getUpgradeHeightWithoutBottom() + UPGRADE_BOTTOM_HEIGHT + 2);
	...
	public Optional<Rect2i> getSortButtonsRectangle() {
		...
		return GuiHelper.getPositiveRectangle(sortButton.getX(), sortButton.getY(), sortByButton.getX() + sortByButton.getWidth() - sortButton.getX(),
	...
	public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		ScNeoForge.EVENT_BUS.post(new ContainerScreenEvent.Render.Foreground(this, guiGraphics, mouseX, mouseY));

		pose.popMatrix();

		if (searchBox != null) {
			searchBox.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
		}

		settingsTabControl.extractForeground(guiGraphics, mouseX, mouseY, partialTicks);
		extractErrorOverlay(guiGraphics);
		if (modalOverlay != null) {
			extractModalOverlay(guiGraphics, mouseX, mouseY, partialTicks);
		}
	}

	private void extractModalOverlay(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		guiGraphics.fill(0, 0, width, height, 0x99000000);
		modalOverlay.extractRenderStateInLatePass(guiGraphics, mouseX, mouseY, partialTicks);
		modalOverlay.extractTooltip(this, guiGraphics, mouseX, mouseY);
	}
	...
	public Slot getHoveredSlot(double mouseX, double mouseY) {
		...
		if (inventoryScrollPanel != null) {
			...
			return slot == null || menu.isStorageInventorySlot(slot) ? null : slot; // if super finds inventory slot that's hidden inside the scroll panel just
																				// return null
		} else {
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		extractStorageTitle(guiGraphics);
		guiGraphics.text(font, playerInventoryTitle, inventoryLabelX, inventoryLabelY, ARGB.opaque(4210752), false);
		extractUpgradeInventoryParts(guiGraphics, mouseX, mouseY);
		extractUpgradeSlots(guiGraphics, mouseX, mouseY);
		if (inventoryScrollPanel == null) {
			extractStorageInventorySlots(guiGraphics, mouseX, mouseY);
		}
		extractPlayerInventorySlots(guiGraphics, mouseX, mouseY);
	}
	...
	public void extractStorageInventorySlots(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean canShowHover) {
		extractSlotsList(guiGraphics, mouseX, mouseY, menu.slots, slot -> !isStorageSlotRenderReplaced(slot.index), canShowHover, 0,
				menu.getNumberOfStorageInventorySlots());
	...
	private void extractPlayerInventorySlots(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		extractSlotsList(guiGraphics, mouseX, mouseY, menu.slots, slot -> true, true, menu.getNumberOfStorageInventorySlots(), menu.slots.size());
	}

	private void extractSlotsList(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, List<Slot> slots, Predicate<Slot> canShow, boolean canShowHover) {
		extractSlotsList(guiGraphics, mouseX, mouseY, slots, canShow, canShowHover, 0, slots.size());
	}
	...
	private void extractSlotsList(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, List<Slot> slots, Predicate<Slot> canShow, boolean canShowHover,
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			if (canShowHover && slot == hoveredSlot) {
				extractSlotHighlightBack(guiGraphics, slot);
			}
			extractSlot(guiGraphics, slot, mouseX, mouseY);
			if (canShowHover && slot == hoveredSlot) {
				extractSlotHighlightFront(guiGraphics, slot);
			}
		}

		if (hoveredSlotBefore != null && hoveredSlotBefore != hoveredSlot) {
			// onStopHovering(hoveredSlotBefore); // removed in MC 26.2 (private in AbstractContainerScreen)
		}
	}

	private void extractUpgradeSlots(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		extractSlotsList(guiGraphics, mouseX, mouseY, menu.upgradeSlots, slot -> slot.x != DISABLED_SLOT_X_POS, true);
	}
	...
	protected void extractSlot(GuiGraphicsExtractor guiGraphics, Slot slot, int mouseX, int mouseY) {
		...
		if (isQuickCrafting && quickCraftSlots.contains(slot) && !carriedStack.isEmpty()) {
			...
			if (StorageContainerMenuBase.canItemQuickReplace(slot, carriedStack) && menu.canDragTo(slot)) {
				...
				int renderCount = StorageContainerMenuBase.getQuickCraftPlaceCount(slot, quickCraftSlots.size(), quickCraftingType, carriedStack)
						+ slotStackCount;
		...
		if (slot instanceof IFilterSlot && !slot.isActive()) {
			extractSlotOverlay(guiGraphics, slot, DISABLED_FILTER_SLOT_COLOR);
		}
	}

	private void extractStack(GuiGraphicsExtractor guiGraphics, int x, int y, ItemStack itemstack, boolean flag, @Nullable String stackCountText) {
		if (flag) {
		...
		if (shouldUseSpecialCountRender(itemstack)) {
			...
			extractStackCount(guiGraphics, stackCountText, x, y);
		} else {
			guiGraphics.itemDecorations(font, itemstack, x, y, stackCountText);
		}
	}

	private void renderSlotBackground(GuiGraphicsExtractor guiGraphics, Slot slot, int i, int j) {
		...
		if (getMenu().isStorageInventorySlot(slot)) {
			if (memorizedStack.isPresent()) {
				guiGraphics.item(memorizedStack.get(), i, j);
				drawStackOverlay(guiGraphics, i, j);
				return;
			} else if (!getMenu().getSlotFilterItem(slot.index).isEmpty()) {
				guiGraphics.item(getMenu().getSlotFilterItem(slot.index), i, j);
				drawStackOverlay(guiGraphics, i, j);
				return;
			}
		}
	...
	private void drawStackOverlay(GuiGraphicsExtractor guiGraphics, int x, int y) {
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, GUI_CONTROLS, x, y, 77, 0, 16, 16, 256, 256);
	}
	...
	private void extractSlotOverlay(GuiGraphicsExtractor guiGraphics, Slot slot, int slotColor) {
		extractSlotOverlay(guiGraphics, slot, slotColor, 0, 16);
	}

	private void extractSlotOverlay(GuiGraphicsExtractor guiGraphics, Slot slot, int slotColor, int yOffset, int height) {
		extractOverlay(guiGraphics, slotColor, slot.x, slot.y + yOffset, 16, height);
	}

	public void extractOverlay(GuiGraphicsExtractor guiGraphics, int slotColor, int xPos, int yPos, int width, int height) {
	...
	protected void drawSlotBg(GuiGraphicsExtractor guiGraphics, int x, int y, int visibleSlotsCount) {
		...
		GuiHelper.renderSlotsBackground(guiGraphics, x + SLOTS_X_OFFSET, y + SLOTS_Y_OFFSET, slotsOnLine, slotRows, remainingSlots);
	}

	private void drawSlotOverlays(GuiGraphicsExtractor guiGraphics) {
		Matrix3x2fStack pose = guiGraphics.pose();
		pose.pushMatrix();
		pose.translate(leftPos, topPos);
		for (int slotNumber = 0; slotNumber < menu.getNumberOfStorageInventorySlots(); slotNumber++) {
			...
			if (!colors.isEmpty()) {
				...
				for (int slotColor : colors) {
					...
					extractSlotOverlay(guiGraphics, menu.getSlot(slotNumber), slotColor & 0x00_FFFFFF | 0x50_000000, yOffset,
	...
	protected void extractTooltip(GuiGraphicsExtractor guiGraphics, int x, int y) {
		...
		if (getMenu().getCarried().isEmpty()) {
			settingsTabControl.extractTooltip(this, guiGraphics, x, y);
		}
		if (sortButton != null) {
			sortButton.extractTooltip(this, guiGraphics, x, y);
		}
		if (sortByButton != null) {
			sortByButton.extractTooltip(this, guiGraphics, x, y);
		}
		if (transferToStorageButton != null) {
			transferToStorageButton.extractTooltip(this, guiGraphics, x, y);
		}
		if (transferToInventoryButton != null) {
			transferToInventoryButton.extractTooltip(this, guiGraphics, x, y);
		}
		if (searchBox != null) {
			searchBox.extractTooltip(this, guiGraphics, x, y);
		}
	...
	protected List<Component> getTooltipFromContainerItem(ItemStack itemStack) {
		...
		if (hoveredSlot != null && hoveredSlot instanceof StorageInventorySlot && hoveredSlot.getMaxStackSize() != itemStack.getMaxStackSize()) {
			ret.add(Component.translatable(TranslationHelper.INSTANCE.translGuiTooltip("stack_count"), Component
					.literal(NumberFormat.getNumberInstance().format(itemStack.getCount())).withStyle(ChatFormatting.DARK_AQUA)
					.append(Component.literal(" / ").withStyle(ChatFormatting.GRAY)).append(Component
							.literal(NumberFormat.getNumberInstance().format(hoveredSlot.getMaxStackSize(itemStack))).withStyle(ChatFormatting.DARK_AQUA)))
					.withStyle(ChatFormatting.GRAY));
		}
		return ret;
	}

	public void drawInventoryBg(GuiGraphicsExtractor guiGraphics, int x, int y, Identifier textureName) {
	...
	private void drawUpgradeBackground(GuiGraphicsExtractor guiGraphics) {
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, GUI_CONTROLS, leftPos - UPGRADE_INVENTORY_OFFSET, topPos + 4, 0, 4, 25, heightWithoutBottom - 4, 256,
				256);
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, GUI_CONTROLS, leftPos - UPGRADE_INVENTORY_OFFSET, topPos + heightWithoutBottom, 0, 198, 25,
				UPGRADE_BOTTOM_HEIGHT, 256, 256);
		...
		for (int slot = 0; slot < numberOfUpgradeSlots; slot++) {
			if (menu.canDisableUpgrade(slot)) {
				...
				guiGraphics.blit(RenderPipelines.GUI_TEXTURED, GUI_CONTROLS, leftPos - UPGRADE_INVENTORY_OFFSET - 4, y, 0, 204 + (previousHasSwitch ? 1 : 0), 7,
						18 - (previousHasSwitch ? 1 : 0), 256, 256);
	...
	public boolean mouseReleased(MouseButtonEvent event) {
		...
		handleQuickMoveAll(event);

		return super.mouseReleased(event);
	}

	private void handleQuickMoveAll(MouseButtonEvent event) {
		Slot slot = getHoveredSlot(event.x(), event.y());
		if (doubleclick && !getMenu().getCarried().isEmpty() && slot != null && event.button() == 0 && menu.canTakeItemForPickAll(ItemStack.EMPTY, slot)
				&& event.hasShiftDown() && !lastQuickMoved.isEmpty()) {
			for (Slot slot2 : menu.slots) {
				tryQuickMoveSlot(event.button(), slot, slot2);
			}
		}
	}
	...
	private void tryQuickMoveSlot(int button, Slot slot, Slot slot2) {
		...
		if (slot2.mayPickup(minecraft.player) && slot2.hasItem() && slot2 instanceof SlotSuppliedHandler s2 && s2.isSameInventory(slot)) {
	...
	@Override
	protected void slotClicked(Slot slot, int slotNumber, int mouseButton, ContainerInput type) {
		if (type == ContainerInput.PICKUP_ALL && !menu.getSlotUpgradeContainer(slot).map(c -> c.allowsPickupAll(slot)).orElse(true)) {
	...
	private void handleInventoryMouseClick(int slotNumber, int mouseButton, ContainerInput type) {
		...
		minecraft.player.connection.send(new ServerboundContainerClickPacket(menu.containerId, menu.getStateId(), Shorts.checkedCast(slotNumber),
				SignedBytes.checkedCast(mouseButton), type, changedSlotStacks, hashedCarried));
	}
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClicked) {
		...
		boolean containerHandled = containerEventHandlerMouseClicked(event, doubleClicked);
		if (containerHandled) {
			return true;
		}
		return superMouseClickedNoContainer(event, doubleClicked, slot);
	}

	private boolean superMouseClickedNoContainer(MouseButtonEvent event, boolean doubleClicked, Slot slot) {
		InputConstants.Key mouseKey = InputConstants.Type.MOUSE.getOrCreate(event.button());
		boolean flag = this.minecraft.options.keyPickItem.matches(mouseKey);
		Slot hoveredSlot = slot != null ? slot : this.getHoveredSlot(event.x(), event.y());
		this.doubleclick = this.lastClickSlot == hoveredSlot && doubleClicked;
		this.skipNextRelease = false;
		if (event.button() != 0 && event.button() != 1 && !flag) {
			// this.checkHotbarMouseClicked(event); // removed in MC 26.2 (private in AbstractContainerScreen)
		} else {
			int i = this.leftPos;
			int j = this.topPos;
			...
			if (k != -1 && !this.isQuickCrafting) {
				if (this.menu.getCarried().isEmpty()) {
					if (this.minecraft.options.keyPickItem.matches(mouseKey)) {
						this.slotClicked(hoveredSlot, k, event.button(), ContainerInput.CLONE);
					} else {
						boolean flag2 = k != -999 && event.hasShiftDown();
						ContainerInput clicktype = ContainerInput.PICKUP;
						if (flag2) {
							this.lastQuickMoved = hoveredSlot != null && hoveredSlot.hasItem() ? hoveredSlot.getItem().copy() : ItemStack.EMPTY;
							clicktype = ContainerInput.QUICK_MOVE;
						} else if (k == -999) {
							clicktype = ContainerInput.THROW;
						}

						this.slotClicked(hoveredSlot, k, event.button(), clicktype);
					}
					...
					} else if (this.minecraft.options.keyPickItem.matches(mouseKey)) {
						this.quickCraftingType = 2;
					}
				}
			}

			this.lastClickSlot = hoveredSlot;
			return true;
		}
		return true;
	}
	...
	public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
		...
		if (isQuickCrafting) {
			if (slot != null && !itemstack.isEmpty() && (itemstack.getCount() > quickCraftSlots.size() || quickCraftingType == 2)
					&& StorageContainerMenuBase.canItemQuickReplace(slot, itemstack) && slot.mayPlace(itemstack) && menu.canDragTo(slot)
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		...
		// Let vanilla handle Bundle scroll-wheel selection first (via BundleMouseActions).
		// Must come before getChildAt so InventoryScrollPanel doesn't intercept the scroll
		// when hovering over a Bundle in the storage grid.
	...
	private boolean isAllowedSlotCombination(Slot slot, ItemStack carried) {
		...
		return quickCraftSlots.contains(slot) || (!(quickCraftSlots.iterator().next() instanceof StorageContainerMenuBase.StorageUpgradeSlot)
	...
	public Optional<Rect2i> getUpgradeSlotsRectangle() {
		return numberOfUpgradeSlots == 0
				? Optional.empty()
				: GuiHelper.getPositiveRectangle(leftPos - UPGRADE_INVENTORY_OFFSET - (!upgradeSwitches.isEmpty() ? 4 : 0), topPos,
						UPGRADE_INVENTORY_OFFSET + 4, getUpgradeHeight());
	}

	private void extractStackCount(GuiGraphicsExtractor guiGraphics, String count, int x, int y) {
		Matrix3x2fStack pose = guiGraphics.pose();
		pose.pushMatrix();
		...
		guiGraphics.text(font, count, (int) ((x + 19 - 2 - (font.width(count) * scale)) / scale), (int) ((y + 6 + 3 + (1 / (scale * scale) - 1)) / scale),
			...
			if (quickCraftingType == 2) {
				...
				for (Slot slot : quickCraftSlots) {
					...
					int quickCraftPlaceCount = Math.min(
							StorageContainerMenuBase.getQuickCraftPlaceCount(slot, quickCraftSlots.size(), quickCraftingType, carriedStack) + slotStackCount,
							maxStackSize);
	...
	private void extractErrorOverlay(GuiGraphicsExtractor guiGraphics) {
		...
		upgradeSlotChangeResult.errorUpgradeSlots().forEach(slotIndex -> {
			...
			upgradeSlotChangeResult.errorInventorySlots().stream().filter(slotIndex -> !inventorySlotsHandledByControls.contains(slotIndex))
					.forEach(slotIndex -> {
						Slot slot = menu.getSlot(slotIndex);
						// noinspection ConstantConditions
						if (slot != null) {
							extractSlotOverlay(guiGraphics, slot, ERROR_SLOT_COLOR);
						}
					});
			...
			extractErrorMessage(guiGraphics, pose, overlayErrorMessage);
		}));
	}

	private void extractErrorMessage(GuiGraphicsExtractor guiGraphics, Matrix3x2fStack pose, Component overlayErrorMessage) {
		pose.pushMatrix();
	...
	private void extractSlotHighlightBack(GuiGraphicsExtractor guiGraphics, Slot slot) {
		if (slot.isHighlightable()) {
			guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_HIGHLIGHT_BACK_SPRITE, slot.x - 4, slot.y - 4, 24, 24);
		}
	}

	private void extractSlotHighlightFront(GuiGraphicsExtractor guiGraphics, Slot slot) {
		if (slot.isHighlightable()) {
	...
	protected boolean isHovering(Slot slot, double mouseX, double mouseY) {
		...
		boolean superResult = super.isHovering(slot, mouseX, mouseY);
		boolean notCovered = getUpgradeSettingsControl().slotIsNotCoveredAt(slot, mouseX, mouseY);
		return superResult && notCovered;
	}
