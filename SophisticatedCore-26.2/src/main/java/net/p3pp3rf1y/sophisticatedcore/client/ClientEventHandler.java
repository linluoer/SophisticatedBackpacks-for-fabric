...
public class ClientEventHandler {
	...
	public static final KeyMapping SORT_KEYBIND = new KeyMapping(TranslationHelper.INSTANCE.translKeybind("sort"),
			InputConstants.Type.MOUSE, MIDDLE_BUTTON, SOPHISTICATEDCORE_CATEGORY);
	public static final KeyMapping TRANSFER_TO_STORAGE_KEYBIND = new KeyMapping(TranslationHelper.INSTANCE.translKeybind("transfer_to_storage"),
			InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_LEFT_BRACKET, SOPHISTICATEDCORE_CATEGORY);
	public static final KeyMapping TRANSFER_TO_INVENTORY_KEYBIND = new KeyMapping(TranslationHelper.INSTANCE.translKeybind("transfer_to_inventory"),
			InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_RIGHT_BRACKET, SOPHISTICATEDCORE_CATEGORY);
	...
	private static void submitCustomGeometry(SubmitCustomGeometryEvent event) {
		// MC 26.2 shim: getLevelRenderState() returns Object and the camera render state is not accessible.
		// The Fabric port is expected to bridge to its own rendering pipeline; no-op for now.
	}
	...
	public static void handleGuiKeyPress(ScreenEvent.KeyPressed.Pre event) {
		InputConstants.Key key = InputConstants.Type.KEYSYM.getOrCreate(event.getKeyEvent());
		if (SORT_KEYBIND.matches(key) && tryCallSort(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		} else if (TRANSFER_TO_STORAGE_KEYBIND.matches(key) && tryCallTransferToStorage(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		} else if (TRANSFER_TO_INVENTORY_KEYBIND.matches(key) && tryCallTransferToInventory(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		}
	}
	...
	public static void handleGuiMouseKeyPress(ScreenEvent.MouseButtonPressed.Pre event) {
		...
		if (SORT_KEYBIND.matches(input) && tryCallSort(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		} else if (TRANSFER_TO_STORAGE_KEYBIND.matches(input) && tryCallTransferToStorage(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		} else if (TRANSFER_TO_INVENTORY_KEYBIND.matches(input) && tryCallTransferToInventory(event.getScreen())) {
			GuiSoundHelper.playButtonClickSound();
			event.setCanceled(true);
		}
	}
	...
	private static boolean tryCallSort(Screen gui) {
		...
		if (mc.player != null && mc.player.containerMenu instanceof StorageContainerMenuBase<?> container && gui instanceof StorageScreenBase<?> screen) {
			...
			double mouseX = mouseHandler.xpos() * mc.getWindow().getGuiScaledWidth() / mc.getWindow().getScreenWidth();
			double mouseY = mouseHandler.ypos() * mc.getWindow().getGuiScaledHeight() / mc.getWindow().getScreenHeight();
