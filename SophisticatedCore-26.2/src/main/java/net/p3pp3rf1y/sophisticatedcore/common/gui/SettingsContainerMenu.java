...
public abstract class SettingsContainerMenu<S extends IStorageWrapper> extends AbstractContainerMenu implements ISyncedContainer, IAdditionalSlotInfoMenu {
	...
	private class ViewOnlyStorageInventorySlot extends ResourceHandlerSlot {
		...
		public ViewOnlyStorageInventorySlot(ResourceHandler<ItemResource> inventoryHandler, int slotIndex) {
			super(inventoryHandler, (index, resource, amount) -> {
				/* noop */}, slotIndex, 0, 0);
		...
		public Identifier getNoItemIcon() {
			return inaccessibleSlots.contains(getContainerSlot()) && !inaccessibleSlotsWithoutOverlay.contains(getContainerSlot())
					? StorageContainerMenuBase.INACCESSIBLE_SLOT_BACKGROUND
					: emptySlotIcons.getOrDefault(getContainerSlot(), null);
		}
	}
	...
	private <C extends ISettingsCategory<?, ?>> SettingsContainerBase<C> instantiateContainer(SettingsContainerMenu<?> settingsContainer, String name,
			C category) {
