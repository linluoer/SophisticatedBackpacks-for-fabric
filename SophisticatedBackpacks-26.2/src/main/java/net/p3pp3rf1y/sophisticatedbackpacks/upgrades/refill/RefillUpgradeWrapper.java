...
public class RefillUpgradeWrapper extends UpgradeWrapperBase<RefillUpgradeWrapper, RefillUpgradeItem>
	...
	public RefillUpgradeWrapper(IStorageWrapper backpackWrapper, ItemStack upgrade, Consumer<ItemStack> upgradeSaveHandler) {
		...
		targetSlots = new HashMap<>(upgrade.getOrDefault(ModDataComponents.TARGET_SLOTS.get(), new HashMap<>()));
	...
	private void saveTargetSlots() {
		upgrade.set(ModDataComponents.TARGET_SLOTS.get(), ImmutableMap.copyOf(targetSlots));
		save();
	}
	...
	public void tick(@Nullable Entity entity, Level level, BlockPos pos) {
		...
		if (!(entity instanceof Player)) {
			level.getEntities(EntityTypes.PLAYER, new AABB(pos).inflate(REFILL_RANGE), p -> true).forEach(this::refillItemFor);
		} else {
	...
	public boolean pickBlock(Player player, ItemStack filter) {
		...
		try (Transaction tx = Transaction.openRoot()) {
			...
			if (canStashHand) {
				...
				player.sendSystemMessage(Component.translatable("gui.sophisticatedbackpacks.status.no_space_for_mainhand_item"));
				return false;
			}
		}
	}
	...
	private boolean canMoveMainHandToInventory(Player player) {
		ResourceHandler<ItemResource> capability = null;
		if (capability == null) {
			return false;
		}
