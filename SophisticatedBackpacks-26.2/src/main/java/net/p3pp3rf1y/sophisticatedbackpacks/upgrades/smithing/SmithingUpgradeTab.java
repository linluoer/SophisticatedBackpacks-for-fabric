...
public class SmithingUpgradeTab extends UpgradeSettingsTab<SmithingUpgradeContainer> {
	...
	private void renderSlotBg(GuiGraphicsExtractor guiGraphics, Slot slot) {
		GuiHelper.renderSlotsBackground(guiGraphics, slot.x + screen.leftPos - 1, slot.y + screen.topPos - 1, 1, 1);
	}
	...
	protected void extractWidget(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		int inputSlotsY = resultSlot.y + screen.topPos;
		int additionalSlotX = getContainer().getAdditionalSlot().x + screen.leftPos;
		int resultSlotX = resultSlot.x + screen.leftPos;
	...
	protected void moveSlotsToTab() {
		...
		templateSlot.x = x - screen.leftPos + 4;
		templateSlot.y = y - screen.topPos + 1 + 24;

		Slot baseSlot = getContainer().getBaseSlot();
		baseSlot.x = templateSlot.x + 18;
		baseSlot.y = y - screen.topPos + 1 + 24;

		Slot additionalSlot = getContainer().getAdditionalSlot();
		additionalSlot.x = baseSlot.x + 18;
		additionalSlot.y = y - screen.topPos + 1 + 24;

		Slot resultSlot = getContainer().getResultSlot();
		resultSlot.x = x - screen.leftPos + getWidth() - 2 - 3 - 18;
		resultSlot.y = y - screen.topPos + 1 + 24;
	}

	private boolean isHoveringRedCross(int mouseX, int mouseY) {
		Slot additionalSlot = getContainer().getAdditionalSlot();
		int arrowX = getArrowX(additionalSlot.x + screen.leftPos, getContainer().getResultSlot().x + screen.leftPos);
		int arrowY = getArrowY(additionalSlot.y + screen.topPos);
	...
	private boolean isHoveringEmptySlot(Slot slot, int mouseX, int mouseY) {
		return mouseX >= slot.x + screen.leftPos && mouseX < slot.x + screen.leftPos + 16 && mouseY >= slot.y + screen.topPos
				&& mouseY < slot.y + screen.topPos + 16 && slot.getItem().isEmpty();
	}
