...
public class AnvilUpgradeTab extends UpgradeSettingsTab<AnvilUpgradeContainer> {
	...
	public AnvilUpgradeTab(AnvilUpgradeContainer upgradeContainer, Position position, StorageScreenBase<?> screen) {
		super(upgradeContainer, position, screen, BackpackTranslationHelper.INSTANCE.translUpgrade("anvil"),
				BackpackTranslationHelper.INSTANCE.translUpgradeTooltip("anvil"));
	...
	protected void extractWidget(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		int inputSlotsY = firstSlot.y + screen.topPos;
		int firstInputSlotX = firstSlot.x + screen.leftPos;
		int secondInputSlotX = getContainer().getSlots().get(1).x + screen.leftPos;
	...
	protected void moveSlotsToTab() {
		...
		firstInputSlot.x = x - screen.leftPos + 4;
		firstInputSlot.y = y + 42 - screen.topPos + 1;

		Slot secondInputSlot = getContainer().getSlots().get(1);
		secondInputSlot.x = x - screen.leftPos + getWidth() / 2 - 9;
		secondInputSlot.y = y + 42 - screen.topPos + 1;

		Slot resultSlot = getContainer().getSlots().get(2);
		resultSlot.x = x - screen.leftPos + getWidth() - 2 - 3 - 18;
		resultSlot.y = y + 42 - screen.topPos + 1;
	}
