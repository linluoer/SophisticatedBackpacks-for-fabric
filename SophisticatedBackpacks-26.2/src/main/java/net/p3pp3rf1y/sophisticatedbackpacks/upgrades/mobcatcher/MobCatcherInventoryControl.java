...
public class MobCatcherInventoryControl extends UpgradeInventoryControlBase {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		ClientPacketDistributor.sendToServer(new MobCatcherReleasePayload(clickedMob.get().id()));
		return true;
	}
	...
	public void extractErrorOverlay(GuiGraphicsExtractor guiGraphics, Set<Integer> errorInventorySlots) {
		...
		for (CapturedMob capturedMob : MobCatcherStorage.getCapturedMobs(backpackWrapper)) {
			...
			getInteriorVisibleBounds(renderBounds.get(), visibleStorageBounds.get()).ifPresent(bounds -> guiGraphics.fill(bounds.x(), bounds.y(),
					bounds.x() + bounds.width(), bounds.y() + bounds.height(), StorageScreenBase.ERROR_SLOT_COLOR));
		}
	}
	...
	public void extractTooltip(StorageScreenBase<?> screen, GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		...
		GuiHelper.extractTooltip(screen, guiGraphics, ItemStack.EMPTY, tooltipLines,
	...
	private void renderCapturedMobArea(GuiGraphicsExtractor guiGraphics, int x, int y, int widthSlots, int heightSlots) {
		int backgroundX = x - 1;
		int backgroundY = y - 1;
	...
	private void renderCapturedMobBackgroundGradient(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height) {
	...
	private void renderReleaseHint(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height) {
		int releaseX = x + width - 14;
		int releaseY = y + height - 14;
		GuiHelper.blit(guiGraphics, releaseX, releaseY, RELEASE_BUTTON_FOREGROUND);
	}

	private void extractCapturedMobEntity(GuiGraphicsExtractor guiGraphics, LivingEntity entity, CapturedMob capturedMob, int x, int y, int width, int height) {
		...
		int screenX = x + screen.leftPos;
		int screenY = y + screen.topPos;
		guiGraphics.entity(renderState, getRenderScale(entity, width, height), translation, new Quaternionf(ENTITY_RENDER_ANGLE),
				new Quaternionf(ENTITY_CAMERA_ANGLE), screenX, screenY, screenX + width, screenY + height);
	}
	...
	private float getRenderTime(CapturedMob capturedMob) {
		return (Minecraft.getInstance().player == null ? 0 : Minecraft.getInstance().player.tickCount) + getUuidFloat(capturedMob.id(), 3) * 200F;
	}
	...
	private Optional<CapturedMob> getHoveredCapturedMob(List<CapturedMob> capturedMobs, double mouseX, double mouseY) {
		return capturedMobs.stream().filter(capturedMob -> isMouseOverVisibleCapturedMob(capturedMob, mouseX, mouseY)).findFirst();
	}
	...
	private boolean isMouseOverVisibleCapturedMob(CapturedMob capturedMob, double mouseX, double mouseY) {
		...
		for (int yOffset = 0; yOffset < capturedMob.height(); yOffset++) {
			for (int xOffset = 0; xOffset < capturedMob.width(); xOffset++) {
				...
				int x = slot.x + screen.leftPos - 1;
				int y = slot.y + screen.topPos - 1;
	...
	private Optional<LivingEntity> getRenderEntity(CapturedMob capturedMob) {
		...
		try {
			Optional<Entity> entity = MobCatcherStorage.getEntityType(capturedMob)
					.map(entityType -> entityType.create(Minecraft.getInstance().level, EntitySpawnReason.EVENT));
			if (entity.isEmpty()) {
				return Optional.empty();
			}
			...
			livingEntity.load(ValueIOHelper.inputFromCompoundTag(Minecraft.getInstance().level.registryAccess(), capturedMob.entityNbt()));
