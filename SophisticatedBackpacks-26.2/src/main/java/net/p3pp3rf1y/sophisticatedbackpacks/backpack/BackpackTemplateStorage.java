...
public class BackpackTemplateStorage extends SavedData {
	private static final SavedDataType<BackpackTemplateStorage> TYPE = new SavedDataType<>(
			Identifier.fromNamespaceAndPath(SophisticatedBackpacks.MOD_ID, "backpack_templates"), BackpackTemplateStorage::new,
			RecordCodecBuilder.create(builder -> builder.group(
					Codec.unboundedMap(Identifier.CODEC, BackpackTemplate.CODEC).fieldOf("backpackTemplates").forGetter(storage -> storage.backpackTemplates))
					.apply(builder, BackpackTemplateStorage::new)), null);
