...
public class BackpackWrapper implements IBackpackWrapper {
	...
	public static IBackpackWrapper fromStack(ItemStack stack) {
		...
		if (!stack.has(ModCoreDataComponents.STORAGE_UUID.get())) {
			return new BackpackWrapper(stack);
		}

		// 缓存命中检测：repository 按 stack identity 缓存 wrapper，命中时 setContentsUuid 已在
		// 首次注册时完成，无需每次 fromStack 都执行 computeIfAbsent + Set.add 的冗余注册
		//（fromStack 是渲染/tick 等热路径的高频调用）。
		boolean cached = StorageWrapperRepository.getExistingStorageWrapper(stack, IBackpackWrapper.class).isPresent();
		...
		/*
		 * TODO try to add uuid based caching in the future UUID uuid = stack.get(ModCoreDataComponents.STORAGE_UUID.get()); if (uuid == null) { return
		...
		if (inventoryHandler == null) {
			inventoryHandler = new BackpackInventoryHandler(getNumberOfInventorySlots() - (getNumberOfSlotRows() * getColumnsTaken()), this,
					getBackpackContents(), () -> {
						...
						if (SidedThreadGroups.isServerThread()) {
	...
	private void setNumberOfInventorySlots(int itemInventorySlots) {
		...
		getBackpackStack().set(ModCoreDataComponents.NUMBER_OF_INVENTORY_SLOTS.get(), itemInventorySlots);
	}
	...
	public Optional<EnergyHandler> getEnergyHandler() {
		if (!energyStorageInitialized) {
			...
			// 修复：原先漏掉置位，导致每次调用都重新执行 stream 查找与包装链重建
			energyStorageInitialized = true;
		}
	...
	public IBackpackWrapper setBackpackStack(ItemStack backpack) {
		...
		if (renderDataHandler == null) {
			...
			renderDataHandler = new RenderDataHandler(
					Optional.ofNullable(backpack.get(ModCoreDataComponents.RENDER_DATA.get())).map(RenderData::copy).orElseGet(RenderData::new), renderData -> {
						backpack.set(ModCoreDataComponents.RENDER_DATA.get(), renderData.copy());
	...
	private void cacheNumberOfInventorySlots(ItemStack backpackStack, int defaultNumberOfInventorySlots) {
		Integer storedNumberOfInventorySlots = backpackStack.get(ModCoreDataComponents.NUMBER_OF_INVENTORY_SLOTS.get());
		...
		if (storedNumberOfInventorySlots == null || storedNumberOfInventorySlots < resolvedNumberOfInventorySlots) {
			backpackStack.set(ModCoreDataComponents.NUMBER_OF_INVENTORY_SLOTS.get(), resolvedNumberOfInventorySlots);
		}
	}

	private void cacheNumberOfUpgradeSlots(ItemStack backpackStack, int defaultNumberOfUpgradeSlots) {
		Integer storedNumberOfUpgradeSlots = backpackStack.get(ModCoreDataComponents.NUMBER_OF_UPGRADE_SLOTS.get());
		...
		if (storedNumberOfUpgradeSlots == null || storedNumberOfUpgradeSlots < resolvedNumberOfUpgradeSlots) {
			backpackStack.set(ModCoreDataComponents.NUMBER_OF_UPGRADE_SLOTS.get(), resolvedNumberOfUpgradeSlots);
		}
	}

	@Nullable
	private UUID cachedContentsUuid = null;
	private boolean contentsUuidResolved = false;
	@Nullable
	private Optional<UUID> cachedContentsUuidOptional = null;

	@Override
	public Optional<UUID> getContentsUuid() {
		// Optional 缓存：该方法在 fromStack/getInventoryHandler/getSettingsHandler 等热路径
		// 反复调用，且 UUID 仅在 getOrCreateContentsUuid/setContentsUuid 时变化
		if (!contentsUuidResolved) {
			cachedContentsUuid = getBackpackStack().get(ModCoreDataComponents.STORAGE_UUID.get());
			cachedContentsUuidOptional = Optional.ofNullable(cachedContentsUuid);
			contentsUuidResolved = true;
		}
		return cachedContentsUuidOptional;
	}
	...
	public int getMainColor() {
		return getBackpackStack().getOrDefault(ModCoreDataComponents.MAIN_COLOR.get(), DEFAULT_MAIN_COLOR);
	}

	@Override
	public int getAccentColor() {
		return getBackpackStack().getOrDefault(ModCoreDataComponents.ACCENT_COLOR.get(), DEFAULT_ACCENT_COLOR);
	}

	@Override
	public Optional<Integer> getOpenTabId() {
		return Optional.ofNullable(getBackpackStack().get(ModCoreDataComponents.OPEN_TAB_ID.get()));
	}

	@Override
	public void setOpenTabId(int openTabId) {
		getBackpackStack().set(ModCoreDataComponents.OPEN_TAB_ID.get(), openTabId);
		backpackSaveHandler.run();
	}

	@Override
	public void removeOpenTabId() {
		getBackpackStack().remove(ModCoreDataComponents.OPEN_TAB_ID.get());
		backpackSaveHandler.run();
	}
	...
	public void setSortBy(SortBy sortBy) {
		getBackpackStack().set(ModCoreDataComponents.SORT_BY.get(), sortBy);
		backpackSaveHandler.run();
	}

	@Override
	public SortBy getSortBy() {
		return getBackpackStack().getOrDefault(ModCoreDataComponents.SORT_BY.get(), SortBy.NAME);
	}
	...
	private ItemStack cloneBackpack(IBackpackWrapper originalWrapper) {
		...
		backpackCopy.remove(ModCoreDataComponents.STORAGE_UUID.get());
	...
	public void setLoot(Identifier lootTableName, float lootFactor) {
		getBackpackStack().set(ModDataComponents.LOOT_TABLE.get(), lootTableName);
		getBackpackStack().set(ModDataComponents.LOOT_FACTOR.get(), lootFactor);
		backpackSaveHandler.run();
	}

	@Override
	public void setTemplate(Identifier templateName) {
		getBackpackStack().set(ModDataComponents.TEMPLATE_NAME.get(), templateName);
	}
	...
	private void fillWithExtraItems(Consumer<ItemStack> insertOrDropItem) {
		...
		int slots = (int) containerItems.allItemsCopyStream().count();
		NonNullList<ItemStack> items = NonNullList.withSize(slots, ItemStack.EMPTY);
		containerItems.copyInto(items);
	...
	public void fillWithLoot(Level level, BlockPos pos, @Nullable Player player) {
		Identifier lootTable = getBackpackStack().get(ModDataComponents.LOOT_TABLE.get());
		if (lootTable == null) {
			return;
		}
	...
	public void fillFromTemplate() {
		ItemStack backpack = getBackpackStack();
		Identifier templateName = backpack.get(ModDataComponents.TEMPLATE_NAME.get());
		...
		backpack.remove(ModDataComponents.TEMPLATE_NAME.get());
	}

	@Override
	public void setContentsUuid(UUID storageUuid) {
		ItemStack backpackStack = getBackpackStack();
		backpackStack.set(ModCoreDataComponents.STORAGE_UUID.get(), storageUuid);
	...
	public void removeContentsUUIDTag() {
		getBackpackStack().remove(ModCoreDataComponents.STORAGE_UUID.get());
		invalidateContentsUuidCache();
	}

	private void invalidateContentsUuidCache() {
		cachedContentsUuid = null;
		cachedContentsUuidOptional = null;
		contentsUuidResolved = false;
	}
	...
	public void setColumnsTaken(int columnsTaken, boolean hasChanged) {
		...
		getBackpackStack().set(ModDataComponents.COLUMNS_TAKEN.get(), columnsTaken);
		if (hasChanged) {
	...
	public int getColumnsTaken() {
		return getBackpackStack().getOrDefault(ModDataComponents.COLUMNS_TAKEN.get(), 0);
	}
	...
	private void fillWithLootFromTable(Level level, BlockPos pos, Identifier lootTable, @Nullable Player player) {
		...
		float lootFactor = getBackpackStack().getOrDefault(ModDataComponents.LOOT_FACTOR.get(), 0f);

		getBackpackStack().remove(ModDataComponents.LOOT_TABLE.get());
		getBackpackStack().remove(ModDataComponents.LOOT_FACTOR.get());
	...
	private void setNumberOfUpgradeSlots(int numberOfUpgradeSlots) {
		...
		getBackpackStack().set(ModCoreDataComponents.NUMBER_OF_UPGRADE_SLOTS.get(), numberOfUpgradeSlots);
	}
	...
	private static class ItemAccessBackpackFluidHandler implements ResourceHandler<FluidResource> {
		...
		@Nullable
		private ResourceHandler<FluidResource> delegate = null;
		...
		private Optional<ResourceHandler<FluidResource>> getDelegate() {
			...
			// delegate 缓存（与 ItemAccessBackpackInventoryHandler 相同模式）：
			// 原实现每次调用都 toStack 拷贝 + new BackpackWrapper + 包装链重建，
			// 自动化设备每 tick 多次查询时是显著的分配与计算开销。
			if (delegate != null && delegateBackpackStack != null && ItemStack.isSameItemSameComponents(backpackStack, delegateBackpackStack)) {
				return Optional.of(delegate);
			}

			delegateBackpackStack = backpackStack;
			delegate = new BackpackWrapper(delegateBackpackStack).getFluidHandler().map(FluidHandlerItemWrapper::new).orElse(null);
		...
		private Optional<MutableBackpackDelegate> getWritableDelegate() {
			...
			return wrapper.getFluidHandler().map(FluidHandlerItemWrapper::new).map(d -> new MutableBackpackDelegate(backpackStack, d));
		}
	...
	private static class ItemAccessBackpackInventoryHandler implements ResourceHandler<ItemResource> {
		...
		private ItemStack getBackpackStack() {
			...
			return accessAmount > 0 && backpackStack.has(ModCoreDataComponents.STORAGE_UUID.get()) ? backpackStack : ItemStack.EMPTY;
		}
