...
public class BackpackScreen extends StorageScreenBase<BackpackContainer> implements IBackpackScreen {
	...
	public boolean keyPressed(KeyEvent event) {
		...
		boolean backpackKeyPressed = KeybindHandler.BACKPACK_OPEN_KEYBIND.matches(InputConstants.getKey(event));
		if (event.key() == 256 || backpackKeyPressed) {
			if (event.key() != 256 && backpackKeyPressed && getFocused() != null && !clearFocusedWidget()) {
				return super.keyPressed(event);
			}
