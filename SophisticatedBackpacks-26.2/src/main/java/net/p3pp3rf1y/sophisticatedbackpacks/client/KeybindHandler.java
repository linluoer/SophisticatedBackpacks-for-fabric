...
public class KeybindHandler {
	...
	public static final KeyMapping BACKPACK_TOGGLE_UPGRADE_5 = new KeyMapping(BackpackTranslationHelper.INSTANCE.translKeybind("toggle_upgrade_5"),
			InputConstants.Type.KEYSYM, KEY_UNKNOWN, SOPHISTICATEDBACKPACKS_CATEGORY);
	...
	public static final KeyMapping BACKPACK_TOGGLE_UPGRADE_2 = new KeyMapping(BackpackTranslationHelper.INSTANCE.translKeybind("toggle_upgrade_2"),
			InputConstants.Type.KEYSYM, KEY_X, SOPHISTICATEDBACKPACKS_CATEGORY);
	public static final KeyMapping BACKPACK_TOGGLE_UPGRADE_1 = new KeyMapping(BackpackTranslationHelper.INSTANCE.translKeybind("toggle_upgrade_1"),
			InputConstants.Type.KEYSYM, KEY_Z, SOPHISTICATEDBACKPACKS_CATEGORY);
	...
	public static final KeyMapping INVENTORY_INTERACTION_KEYBIND = new KeyMapping(BackpackTranslationHelper.INSTANCE.translKeybind("inventory_interaction"),
			InputConstants.Type.KEYSYM, KEY_C, SOPHISTICATEDBACKPACKS_CATEGORY);
	public static final KeyMapping BACKPACK_OPEN_KEYBIND = new KeyMapping(BackpackTranslationHelper.INSTANCE.translKeybind("open_backpack"),
			InputConstants.Type.KEYSYM, KEY_B, SOPHISTICATEDBACKPACKS_CATEGORY);
	...
	public static void handleGuiKeyPress(ScreenEvent.KeyPressed.Pre event) {
		...
		if (BACKPACK_OPEN_KEYBIND.matches(key) && sendBackpackOpenOrCloseMessage()) {
	...
	public static void handleGuiMouseKeyPress(ScreenEvent.MouseButtonPressed.Pre event) {
		...
		if (BACKPACK_OPEN_KEYBIND.matches(input) && sendBackpackOpenOrCloseMessage()) {
			event.setCanceled(true);
		}
	}
	...
	private static boolean sendBackpackOpenOrCloseMessage() {
		...
		if (screen instanceof AbstractContainerScreen<?> containerScreen) {
			...
			if (slot != null && slot.container instanceof Inventory) {
				Optional<String> handlerName = getPlayerInventoryHandlerName(slot.getContainerSlot());

				if (handlerName.isPresent() && slot.getItem().getItem() instanceof BackpackItem) {
					ClientPacketDistributor.sendToServer(new BackpackOpenPayload(slot.getContainerSlot(), "", handlerName.get()));
					return true;
				}
			}
