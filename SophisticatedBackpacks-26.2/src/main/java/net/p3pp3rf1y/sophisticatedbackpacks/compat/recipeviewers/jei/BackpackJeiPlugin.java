...
public class BackpackJeiPlugin implements IModPlugin {
	...
	public void registerRecipes(IRecipeRegistration registration) {
		// 将 catalog 中通过 addCraftingRecipe 添加的配方（基础背包合成、背包层级升级等转换而来的 ShapedRecipe）注册到 JEI。
		// 不这样做的话，这些配方只会留在 catalog 内部，JEI 不会显示（CraftingDisplayCatalogRecipeManagerPlugin 只处理 craftingSpecs，不处理 craftingRecipes）。
		...
		registration.addRecipes(RecipeTypes.CRAFTING,
				DyeRecipesMaker.getSingleColorRecipeSpecs().stream()
						.filter(spec -> spec.sourceStacks().stream().anyMatch(stack -> stack.is(ModItems.BACKPACK.get())))
						.map(SingleColorDyeRecipeSpec::recipeHolder).map(BackpackJeiPlugin::castCraftingRecipeHolder).toList());
		// 直接注册锻造台配方到 JEI，因为 SmithingSpecRecipeManagerPlugin.getAllRecipes() 返回空列表，
		// 导致 catalog 中的 smithingSpecs 不会出现在 JEI 锻造台分类列表中（下界合金背包升级配方等）。
		registration.addRecipes(RecipeTypes.SMITHING,
				ClientRecipeHelper.transformAllRecipeHoldersOfType(RecipeType.SMITHING, SmithingBackpackUpgradeRecipe.class,
						(id, recipeHolder) -> new RecipeHolder<SmithingRecipe>(recipeHolder.id(), recipeHolder.value())));
	}
	...
	private static boolean canShowDyeUsagesFor(ItemStack stack) {
		return stack.getItem() instanceof BackpackItem && !stack.has(ModCoreDataComponents.MAIN_COLOR.get()) && !stack.has(ModCoreDataComponents.ACCENT_COLOR.get())
				&& !stack.has(ModCoreDataComponents.RENDER_DATA.get());
	}

	private static boolean canShowDyeRecipesFor(ItemStack stack) {
		if (!stack.is(ModItems.BACKPACK.get()) || !stack.has(ModCoreDataComponents.MAIN_COLOR.get()) || !stack.has(ModCoreDataComponents.ACCENT_COLOR.get())) {
			return false;
		}

		int mainColor = stack.get(ModCoreDataComponents.MAIN_COLOR.get());
		int accentColor = stack.get(ModCoreDataComponents.ACCENT_COLOR.get());
