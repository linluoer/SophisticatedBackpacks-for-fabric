...
public class PlayerInventoryProvider {
	...
	private PlayerInventoryProvider() {
		...
		addPlayerInventoryHandler(ARMOR_INVENTORY, player -> PlayerInventoryHandler.SINGLE_IDENTIFIER, (player, identifier) -> 1,
	...
	public Optional<RenderInfo> getBackpackFromRendered(Player player, boolean forLayerRendering) {
		for (String handlerName : renderedHandlers) {
			...
			for (String identifier : invHandler.getIdentifiers(player)) {
				// 槽位数在循环内不变，提取到循环外避免每迭代重复调用（36 格物品栏为 37 次 lambda 链调用）
